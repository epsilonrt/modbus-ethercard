/*
    ModbusEthercard.cpp - Source for Modbus TCP using Ethercard ENC28J60 Library
...
ModbusEthercard::ModbusEthercard() {
  ether.hisport = MODBUSIP_PORT;
}

void ModbusEthercard::config (uint8_t * mac) {
  ether.begin (sizeof Ethernet::buffer, mac, ENC28J60_CS);
  ether.dhcpSetup();
}

void ModbusEthercard::config (uint8_t * mac, uint8_t * ip) {
  ether.begin (sizeof Ethernet::buffer, mac, ENC28J60_CS);
  ether.staticSetup (ip);
}

void ModbusEthercard::config (uint8_t * mac, uint8_t * ip, uint8_t * dns) {
  ether.begin (sizeof Ethernet::buffer, mac, ENC28J60_CS);
  ether.staticSetup (ip, 0, dns);
}

void ModbusEthercard::config (uint8_t * mac, uint8_t * ip, uint8_t * dns, uint8_t * gateway) {
  ether.begin (sizeof Ethernet::buffer, mac, ENC28J60_CS);
  ether.staticSetup (ip, gateway, dns);
}

void ModbusEthercard::config (uint8_t * mac, uint8_t * ip, uint8_t * dns, uint8_t * gateway, uint8_t * subnet) {
  ether.begin (sizeof Ethernet::buffer, mac, ENC28J60_CS);
  ether.staticSetup (ip, gateway, dns, subnet);
}

void ModbusEthercard::task() {
